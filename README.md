...
**模型工作中也能输入(排队)**:干活期间输入框照常可用(不再变灰,标签提示 `Enter 排队`),按 `Enter` 不会被丢弃——消息区提示 `⏳ 已排队`,模型**下一个轮边界**就会带着它继续干(状态栏显示 `队列 N`)。排队消息**不会打断**正在跑的回合;想立刻停下当前的活按 `Esc`,队列里剩下的紧接着发出。
