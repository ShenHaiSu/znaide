...
**模型工作中也能输入(排队)**:干活期间输入框照常可用(不再变灰,标签提示 `Enter 排队`),按 `Enter` 不会被丢弃——消息进队列(状态栏显示 `队列 N`),模型**下一次调用**就带着它继续干。排队不会打断正在跑的回合;想立刻停下当前的活按 `Esc`,队列里剩下的紧接着发出。
...
**底部状态栏**从左到右显示:`权限模式(中文短名:询问 / 编辑放行 / 全自动 / 超级)[| 人格(有人格时显示,空 = 默认助手)] | 模型 | 状态(就绪/工作中…/压缩中…;执行=流动光条、压缩=收纳推进条动画) · 轮 本轮已用/上限(上限 0 显示 ∞,见 `max_turns`)[| token 统计:**累计** in 输入 · out 输出 · Σ 总计(计费口径累计:每次模型往返都重发整段历史、各计一次,所以随轮数增长,**不是上下文占用**;`/clear`、`/resume` 归零,`/compact` 不归零;流式中 out/Σ 带 ≈ 实时估算)][| ctx ▓▓▓▓▓░░░░░ 34% 上下文占用条(占模型窗口比例,窗口已知即常驻;70% 变黄 / 90% 变红并提示可 /compact;**占用还没测到时显示 `ctx ?`**——刚 `/resume`、`/compact` 完或还没发过消息,真实值等下一轮回复;/clear 后是真的空才画 0%;**窗口认不出来时显示 `ctx ~21.4k 窗口未知`**:只报绝对量、不编百分比、不提示压缩)] · 会话 <id>`。

**输入框与输出框边框颜色 = 当前权限模式**:询问=绿 / 编辑放行=天蓝 / 全自动=紫 / 超级(YOLO)=红(工作中也保持该颜色,输入框不再变灰;输出框常显);`Shift+Tab` 切换后边框立即变色。会话 ID 启动即显示,与本会话历史文件同名(`~/.znaide/sessions/<id>.jsonl`),可配合 `/resume` 用 ID 片段精确定位恢复。
...
| `/config` | 打开配置面板(服务商 → 模型 → API Key → 上下文窗口 → 轮数上限 → 验证;重开时带出当前已配置的值,顶部显示"当前配置"摘要;改完立即生效) |
| `/undo` `/undo <序号>` | 列出快照 / 回滚到指定版本 |
| `/resume` `/resume <序号\|片段>` | **无参**:打开全屏**会话管理窗口**(「历史会话/长期记忆」页签:↑↓ 选择、空格多选、`d` 批量删除、`n` 改备注、`/` 过滤(Enter 保留筛选 / Esc 清除)、Enter 恢复/查看,当前会话禁删);**带参**:直接恢复该历史会话(`[无头]` = 命令行 `-p` 产生) |
...
| `/update` | 检查并更新到最新版本(启动时自动检查一次,发现新版会提示;也可用 `znaide --update`)。**双源自动回退**:默认 Gitee 镜像源(国内无需代理、直连快),连不上自动切 GitHub;GitHub 源可设 `HTTPS_PROXY` 加速。Linux/macOS 替换后下次启动生效,Windows 退出程序后自动完成替换 |
...
**技能 = 可被模型自主调用、也可手动 `/名字` 触发的能力包**。模型判断任务匹配某个技能时,会调用 `skill` 工具并把技能名带进来;系统把说明书正文(渲染好 `{args}`)+ 可选入口脚本的输出注入上下文,模型按说明用基础工具逐步完成。技能不新增权限面——所有落地动作仍走权限档位、高危命令判定与 undo 快照。
