...
**模型工作中也能输入(排队)**:干活期间输入框照常可用(不再变灰,标签提示 `Enter 排队`),按 `Enter` 不会被丢弃——消息区提示 `⏳ 已排队`,等**本回合结束后自动逐条发送**(状态栏显示 `队列 N`)。排队消息**不会打断**正在跑的回合;想立刻停下当前的活按 `Esc`,中断后队列紧接着接手。
